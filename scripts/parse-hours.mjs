...
export function parseOpeningHoursString(s) {
  ...
  for (const part of parts) {
    ...
    const m = trimmed.match(
      /^([A-Za-z]+)\s+(\d{1,2}:\d{2}(?::\d{2})?(?:\s*[AaPp][Mm])?)\s*-\s*(\d{1,2}:\d{2}(?::\d{2})?(?:\s*[AaPp][Mm])?)$/,
    );
    if (!m) continue;
 ...
 * Closed-only blocks (lockers, etc.) are skipped. Ambiguous remaining blocks
 * (more than one complete week) are rejected unless JSON-LD already won.
 * @param {string} html
 * @returns {{ day: string, open: string, close: string }[]}
 */
